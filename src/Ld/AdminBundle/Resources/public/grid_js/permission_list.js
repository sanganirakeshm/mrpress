$(document).ready(function () {
	dTable = $('#permission_grid').dataTable({
				"oLanguage": {
                ...
                aoColumnDefs: [
							...
							},
                            {
		                        "mRender": function(data, type, row) {
		                            ...
		                            if(permissionEditPath) {
		                            	var editUrl = permissionEditPath.replace("pid", pId);                            
		                            ...
		                            if(permissionActiveInactivePath) {
		                            	returnData +='<a class="btn '+activeInactiveBtnClass+' btn-sm" href="javascript:void(0)" onclick="ajaxActiveInactive(\''+permissionActiveInactivePath+'\','+pId+','+enabled+')"><i class="'+activeInactiveIconClass+' m-r-0-5"></i>'+activeInactiveBtnText+'</a>';
		                            }
		                            if(permissionDeletePath) {
		                            	returnData +='<a class="btn btn-danger btn-sm" href="javascript:void(0)" onclick="ajaxDeleteRecord(\''+permissionDeletePath+'\','+pId+',\'\',\'\')"><i class="ti-trash m-r-0-5"></i>Delete</a>';
		                            }
	...
	$('#permission_grid').dataTable().columnFilter({ 	
    ...
    $('#permission_grid').on( 'search.dt', function () { $( ".preloader" ).hide(); } ).DataTable();
});